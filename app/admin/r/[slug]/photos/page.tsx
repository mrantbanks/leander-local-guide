...
export default async function AdminPhotos({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const ph = await pool.query('select id, filename, caption, is_menu from photos where place_id = (select id from restaurants where slug = $1) order by sort, created_at', [slug]);
  ...
  return (
    <main className="max-w-3xl mx-auto px-5 py-8">
      ...
      <PhotoStudio slug={slug} initial={ph.rows.map((p) => ({ id: p.id, filename: p.filename, caption: p.caption, isMenu: p.is_menu }))} />
    </main>
  );
}
