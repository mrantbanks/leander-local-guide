  ...
  return (
    <section className="border-b border-rule bg-paper-raised">
      <div className="max-w-6xl mx-auto px-5 py-2.5 flex items-center gap-4">
        ...
        <span className="hidden sm:inline font-stamp uppercase tracking-[0.1em] text-[12px] text-ink-soft whitespace-nowrap shrink-0">{dateline}</span>
        <div className="flex items-center gap-5 overflow-x-auto no-scrollbar">
          {items.map((it, i) => (
            <Link key={it.slug + i} href={`/r/${it.slug}`} className="group flex items-center gap-2 whitespace-nowrap shrink-0">
              <span className={`font-stamp uppercase tracking-[0.06em] text-[12px] px-1.5 py-0.5 border rounded-[2px] -rotate-1 ${TAG[it.kind].cls}`}>{TAG[it.kind].label}</span>
