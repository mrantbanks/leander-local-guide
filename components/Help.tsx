  ...
  return (
    <span ref={ref} className="relative inline-flex align-middle">
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((o) => !o)}
        className="inline-grid place-items-center w-[15px] h-[15px] ml-1 rounded-full border border-chile text-chile text-[10px] leading-none normal-case tracking-normal hover:bg-chile hover:text-paper align-middle"
      >
        ?
      </button>
      {open && (
        ...
        >
          ...
          {example && (
            <span className="block mt-1.5 font-ui text-xs text-ink-soft leading-relaxed">
              <b className="font-stamp uppercase tracking-[0.06em] text-chile text-[11px]">e.g.</b> {example}
            </span>
          )}
        </span>
      )}
